...
*/
  findCoordinates = (p = true) => {
    Geolocation.getCurrentPosition(
      position => {
        ...
        console.log('FD ' + this.state.latitude + '-' + this.state.longitude);
        /* update */
        if (p) {
          this.forceUpdate();
        }
      },
      error => Alert.alert('INN' + error.message),
      {enableHighAccuracy: true, timeout: 20000},
    );
  };

  markers() {
    console.log('MK2 ' + this.state.latitude + '-' + this.state.longitude);
    if (this.state.latitude !== 0 && this.state.longitude !== 0) {
      console.log('MK' + this.state.latitude + '-' + this.state.longitude);
      return (
        <MapView.Marker
          coordinate={{
          ...
          title={'Location'}
          description={'via la géolocalisation'}
        />
      );
    }
  }
  ...
  createMarkers(request) {
    let code = '';
    request.map(parking => {
      //const parkJson = JSON.parse(parking);
      console.log(parking);
      let park =
        'L.marker([' +
        parking.latitude +
        ',' +
        parking.longitude +
        ',{opacity:0.75}]).addTo(mymap).bindPopup("' +
        parking.nom +
        '").openPopup();';
      code += park;
    });
  ...
  render() {
    console.log('Render');
    const j4code = 'mymap.setView([0, 0], 10)'; /* works ! */
    if (this.state.latitude === 0) {
      this.findCoordinates(false);
    }
    let j5code =
      'mymap.setView([' +
      this.state.latitude +
    ...
    let j6code = this.createMarkers(json1);

    console.log('java code : ' + j5code);
    return (
      <View>
